...
yp.use(['bootstrap'], function() {
  yp.ready(function() {
    ...
    , view: function() {
        ...
        /* 个人资料获取 */
        yp.ajax(oConfig.oUrl.userGet, { type: 'get', data: { uid: oConfig.oData.uid, permission: oConfig.oData.permission }})
          .done(function(msg) {
          // 判页面能否编辑
        ...
        var $skillAccuracy;
        var $skillContent = '';
        yp.ajax(oConfig.oUrl.skillGet, { type: 'get', data: { uid: oConfig.oData.uid }})
          .done(function(msg) {
          if (msg.code == 0) {
            for (var i = 0; i < msg.data.length; i++) {
              $skillAccuracy = msg.data[i].accuracy * 100;
              $skillContent = '';
              $skillContent += '<li><div class="skill-name col-sm-3 control-label">'
                             + msg.data[i].skillname
                             + '</div><div class="skill-progress col-sm-9"><div class="progress"><div class="progress-bar" role="progressbar" aria-valuenow="'
                             + $skillAccuracy
