...
var CodeMirror = (function() {
  ...
  function updateScrollbars(cm) {
    ...
    var needsH = d.scroller.scrollWidth > (d.scroller.clientWidth + 220);
  ...
  function onKeyUp(e) {
    ...
    if($('.wiki-content').length > 0 ) {
      var parse = new Editor().constructor.markdown;
      var text = cm.getValue();
      $('.wiki-content').html(parse(text));
      // $('head').append('<script src="../j/skill/markdown/highlight.pack.js"></script>');
  ...
  function onKeyDown(e) {
    ...
    //当前行数高亮
    // var hCount = +$('.CodeMirror-cursor').eq(0).css('top').split('px')[0] / 14;

    // $('.CodeMirror-code').find('pre.pre-hover').removeClass('pre-hover');
    // $('.CodeMirror-code').find('pre').eq(hCount).addClass('pre-hover');

    // $('.CodeMirror-code').find('pre.pre-hover').removeClass('pre-hover');
    // console.log($('.CodeMirror-code').find('pre').eq(cm.getCursor().line).hasClass('pre-hover'))
    // if(!$('.CodeMirror-code').find('pre').eq(cm.getCursor().line).hasClass('pre-hover')) {
    //   setTimeout(function() {
        // $('.CodeMirror-code').find('pre').eq(cm.getCursor().line).addClass('pre-hover')
    //   },100)
    // }

    //同步高度
    if($('.CodeMirror-code').length > 0 && $('.wiki-content').length > 0) {

      $('.wiki-content').css('height','auto');
      var contentHeight = $('.wiki-content').height();
      $('.CodeMirror-code').css('height','auto');
      var editorHeight =  $('.CodeMirror-code').height();
      if(contentHeight > editorHeight) {
        $('.CodeMirror-code').height(contentHeight)
      }else{
        $('.wiki-content').height(editorHeight)
      }
  ...
  // $(document).on('mousedown','.CodeMirror-code pre',function() {
  //     //当前行数高亮
  //     var hCount = +$('.CodeMirror-cursor').eq(0).css('top').split('px')[0] / 14;
  ...
  // $(document).on('click','.CodeMirror pre',function(){
  //   $('.CodeMirror pre').removeClass('pre-hover');
 ...
 function preHoverFocus(line,enter) {
  return;
  if(enter) line = line + 1;
    setTimeout(function() {
      $('.CodeMirror-code').find('pre.pre-hover').removeClass('pre-hover');
      $('.CodeMirror-code').find('pre').eq(line).addClass('pre-hover');
    },100);
  }
 ...
 */
function createIcon(name, options, title) {
  options = options || {};
...
var toolbar = [
  {name: 'bold', action: toggleBold, title : '加粗 Ctrl+B' },
 ...
 * get markcode.
 */
Editor.prototype.getMarkCode = function() {
  var cm = this.codemirror;
  var text = cm.getValue();
...
Editor.prototype.render = function(el) {
  ...
  if(contentHeight > 500) {
    $('.CodeMirror-code').css('min-height',contentHeight);
  }

  //初始化行数 字数
  $('.words').text(this.codemirror.getValue().length);
  $('.lines').text(this.codemirror.lineCount());
};
...
Editor.prototype.createToolbar = function(items) {
  ...
  for (var i = 0; i < items.length; i++) {
    (function(item) {
      var el;
      if (item.name) {
        el = createIcon(item.name, item, item.title);
      } else if (item === '|') {
        el = createSep();
      } else {
        el = createIcon(item);
