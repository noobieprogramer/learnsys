  ...
  , bindEvent: function(){
      ...
      self.$element.on('click', '.pageItem', function(){
        ...
        if(opts.onPageChange){
          self.$element.html(self.fGetPagination());
          // self.$element.find('.pageGo').focus();
          opts.onPageChange(opts.curPage, $this.attr('href'), self.$element);
          return false;
        }
        if(opts.formId){
          var $form = $('#' + opts.formId);
          $form.find('.page').val(opts.curPage);
          $form[0].submit();
          return false;
        }
      });
      ...
      function fGo(){
        ...
        opts.curPage = page;
        var url = self.fGetUrl(opts.curPage);
        if(opts.onPageChange){
          self.$element.html(self.fGetPagination());
          // self.$element.find('.pageGo').focus();
          opts.onPageChange(opts.curPage, url, self.$element);
          return false;
        }
        if(opts.formId){
          var $form = $('#' + opts.formId);
