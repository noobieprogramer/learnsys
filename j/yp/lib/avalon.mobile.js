                            ...
                            if (!isEqual(value, neo)) {
                                if (rchecktype.test(valueType)) {
                                    if ("value" in accessor) { //如果已经转换过
                                        ...
                                        fn && fn()
                                        (typeof vmodel.$fire === 'function') && vmodel.$fire(name, value, preValue)///
        ...
        if (four === "duplex") {
            ...
            if (data.type === "on") {
                if (code.indexOf(".bind(") === -1) {
                    ///
                    code = code.replace(/([^\)]+?)(\.[^\)\.]*)?$/, function($0, $1, $2) {
                        return $0 + '.bind(' + ( $2 ? $1 : names[0]) + ')'
                    })
                    ///
    ...
    var modelBinding = bindingHandlers.duplex = bindingHandlers.model = function(data, vmodels) {
        ...
        if (typeof modelBinding[tagName] === "function" && vmodels && vmodels.length) {
            ...
            if (array) {
                ...
                if (vm) var updateView = modelBinding[tagName](data, array[0], vm)///
            }
            if (!updateView) {
                updateView = function() {
                }
